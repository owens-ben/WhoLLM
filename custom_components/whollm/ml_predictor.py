...
class MLPredictor:
    ...
    def _load_models(self) -> bool:
        ...
        if MODEL_PATH.exists():
            try:
                ...
                self.models = {name: info["model"] for name, info in data.get("models", {}).items()}
    ...
    def extract_features(self, context: dict, entity_name: str) -> list[float]:
        ...
        ai_person = any(v.get("state") == "on" and v.get("detection_type") == "person" for v in ai_detection.values())
        ai_animal = any(v.get("state") == "on" and v.get("detection_type") == "animal" for v in ai_detection.values())
        ...
        features = {
            ...
            "light_living_room": 1.0 if any("living" in light.lower() for light in lights_on) else 0.0,
            "light_bedroom": 1.0 if any("bedroom" in light.lower() for light in lights_on) else 0.0,
            "light_office": 1.0 if any("office" in light.lower() for light in lights_on) else 0.0,
            "light_kitchen": 1.0 if any("kitchen" in light.lower() for light in lights_on) else 0.0,
            "light_bathroom": 1.0 if any("bath" in light.lower() for light in lights_on) else 0.0,
    ...
    def predict(
        ...
        try:
            ...
            indicator_idx = (
                self.feature_names.index("indicator_count") if "indicator_count" in self.feature_names else -1
            )
            ...
            probabilities = {cls: float(p) for cls, p in zip(classes, proba, strict=False)}
